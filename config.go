...
// the Raft server
type Config struct {
	EpochDuration  time.Duration  	//每个epoch 的时间间隔

	MaxAppendEntries int   		//一次发送log 数量

	MaxConnPoolNum	int   		//transport 中最大缓存的connection

	MaxSchedulerWorkerNum   int
...
func DefaultConfig() *Config {
	return &Config{
		EpochDuration:		    30 * time.Millisecond,
		MaxConnPoolNum:		    4,
		MaxAppendEntries:           64,
		ConnectTimeout:             10 * time.Second,
		MaxSchedulerWorkerNum:	    16,
